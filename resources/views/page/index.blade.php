...
<div class="border rounded mt-5 mx-auto d-flex flex-column align-items-stretch bg-white" style="width: 380px;">
        <div class="d-flex justify-content-between flex-shrink-0 p-3 link-dark  border-bottom">
            <span class="fs-5 fw-semibold">Total Daftar Mahasiswa {{$data->total()}}</span>
            <a href="{{url('/buat')}}" class="btn btn-sm btn-primary">add</a>
        </div>
        @foreach($data as $item)
        <div class="list-group list-group-flush border-bottom scrollarea">
            <div class="list-group-item list-group-item-action py-3 lh-tight" aria-current="true">
                <div class="d-flex w-100 align-items-center justify-content-between">
                    <strong class="mb-1">Nama: {{$item->nama}}</strong>
                    <small>Wed</small>
                </div>
                <div class="col-10 mb-1 small">Jurusan: {{$item->jurusan}}</div>
                <div class="col-10 mb-1 small">Usia: {{$item->usia}}</div>
                <div class="group-action">
                    <form action="{{url("/data/$item->id")}}" method="POST">

                    <a href="{{url("edit/$item->id")}}" class="badge bg-info text-white">edit</a>
                    @csrf @method('delete')
